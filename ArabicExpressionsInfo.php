...
/**
 * Represents information about a single expression.
 */
class ArabicExpressionsInfo
{
    const SYMBOLS_BASE_CLASS_NAME = 'as-';

    /**
     * The regex matching this expression.
     * @var string
     */
    private $m_regex;

    /**
     * The regex replacement HTML code of this expression.
     * @var string
     */
    private $m_htmlCode;
    ...
    public function __construct($name, $textRepresentations)
    {
        ...
        //   trailing whitespace) so that multiple consecutive expressions are detected correctly (see issue #5).
        // NOTE 2: All HTML tags have been escaped at this point (with "DELIM_CHARS" being the marker).
        $this->m_regex = '/(?<=\s|^|' . ArabicExpressionsPlugin::DELIM_CHARS . ')'
                       . '(?:' . $this->m_regex . ')'
                       . '(?=\s|$|' . ArabicExpressionsPlugin::DELIM_CHARS . ')/U';

        $this->m_htmlCode = '<span class="' . self::SYMBOLS_BASE_CLASS_NAME . $name . '"></span>';
    }

    public function replaceTextExpressions($postText)
    {
